...
/// PdmListField
//--------------------------------------------------------------------------------------------------
TEST(BaseTest, PdmListFieldOperations)
{
...
public:
    ...
    // Fields
    caf::PdmField<PdmObject*> m_pointersField;
    caf::PdmPointersField<SimpleObj*> m_simpleObjPtrField2;
};
...
TEST(BaseTest, PdmReferenceHelper)
{
    ...
    {
        caf::ReferenceHelper* refHelper = caf::ReferenceHelper::instance();
        QString refString = refHelper->referenceFromRootToObject(s3);
        EXPECT_TRUE(refString.isEmpty());

        refHelper->setRoot(ihd1);
        refString = refHelper->referenceFromRootToObject(s3);
        QString expectedString = ihd1->m_simpleObjectsField.keyword() + " 3";
        EXPECT_STREQ(refString.toAscii(), expectedString.toAscii());

        caf::PdmObject* fromRef = refHelper->objectFromReference(refString);
    ...
    // Test reference to field
    {
        caf::ReferenceHelper* refHelper = caf::ReferenceHelper::instance();
        refHelper->setRoot(objA);
        QString refString = refHelper->referenceFromRootToField(&(ihd1->m_simpleObjectsField));

        caf::PdmFieldHandle* fromRef = refHelper->fieldFromReference(refString);
