...
PdmUiTreeOrdering* PdmObject::uiTreeOrdering(QString uiConfigName /*= ""*/)
{
    ...
    PdmUiTreeOrdering* uiTreeOrdering = new PdmUiTreeOrdering(NULL, this);
...
//--------------------------------------------------------------------------------------------------
void PdmObject::addDefaultUiTreeChildren(PdmUiTreeOrdering* uiTreeOrdering)
{
...
void PdmObject::expandUiTree(PdmUiTreeOrdering* root, QString uiConfigName /*= "" */)
{
    ...
    else //( root->childCount() == 0) // This means that no one has tried to expand it.
    {
        ...
        {
            if (root->isRepresentingField() && !root->field()->isUiChildrenHidden(uiConfigName))
            {
                ...
                {
                    PdmObject* childObject = fieldsChildObjects[cIdx];
                    if (childObject)
                    {
                        root->appendChild(childObject->uiTreeOrdering(uiConfigName));
                    }
                }
            }
            else if (root->isRepresentingObject())
            {
                ...
                root->object()->addDefaultUiTreeChildren(root);
                if (root->childCount())
                {
                    expandUiTree(root);
                }
