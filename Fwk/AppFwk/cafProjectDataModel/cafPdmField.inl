...
{
    PdmObject* obj = m_pointers[index].rawPtr();
    if (obj)
    {
...
//--------------------------------------------------------------------------------------------------
template<typename DataType>
QString PdmPointersField<DataType*>::containedClassKeyword()
{
    // Getting the typeid will return a string like this
    //   class caf::PdmPointersField<class SmallDemoPdmObjectA * __ptr64> * __ptr64
    //
    QString typeNameString = qStringTypeName(this);

    QString searchString("<class ");
    int startPos = typeNameString.indexOf(searchString);
    if (startPos != -1) startPos += searchString.length();
        
    int endPos = typeNameString.indexOf(" *");

    if (startPos != -1 && endPos != -1)
    {
        QString keyword = typeNameString.mid(startPos, endPos - startPos);
        return keyword;
    }
...
//--------------------------------------------------------------------------------------------------
template<typename DataType>
PdmObject* PdmPointersField<DataType*>::createAppendObject(int indexAfter)
{
    QString classKeyword = this->containedClassKeyword();
...
//--------------------------------------------------------------------------------------------------
template<typename DataType>
void PdmPointersField<DataType*>::insertAt(size_t indexAfter, PdmObject* obj)
{
    typename std::vector< PdmPointer<DataType> >::iterator it;
