...
void PdmUiTreeViewModel::setPdmItemRoot(PdmUiItem* rootItem)
{
    ...
    PdmFieldHandle* field = dynamic_cast<PdmFieldHandle*> (rootItem);
    if (field)
    {
        newRoot = new PdmUiTreeOrdering(field);
        PdmObject::expandUiTree(newRoot, m_uiConfigName);
    }
    else
    {
        PdmObject * obj = dynamic_cast<PdmObject*> (rootItem);
        if (obj)
        {
...
void PdmUiTreeViewModel::updateSubTree(PdmUiItem* pdmRoot)
{
    ...
    PdmFieldHandle* field = dynamic_cast<PdmFieldHandle*> (pdmRoot);
    if (field)
    {
        newTreeRootTmp = new PdmUiTreeOrdering(field);
    }
    else
    {
        PdmObject* obj = dynamic_cast<PdmObject*> (pdmRoot);
        if (obj)
        {
            newTreeRootTmp = new PdmUiTreeOrdering(obj);
        }
    }

    PdmObject::expandUiTree(newTreeRootTmp, m_uiConfigName);
...
bool PdmUiTreeViewModel::setData(const QModelIndex &index, const QVariant &value, int role /*= Qt::EditRole*/)
{
    ...
    PdmObject* obj = treeItem->object();

    if (role == Qt::EditRole && obj->userDescriptionField())
    {
        PdmCommandFeaturesCore::instance()->fieldChangedCommand(obj->userDescriptionField(), value);
    ...
    else if (role == Qt::CheckStateRole && obj->objectToggleField())
    {
        ...
        PdmCommandFeaturesCore::instance()->fieldChangedCommand(obj->objectToggleField(), toggleOn);
...
Qt::ItemFlags PdmUiTreeViewModel::flags(const QModelIndex &index) const
{
    ...
    {
        PdmObject* pdmObject = treeItem->object();
